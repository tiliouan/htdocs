	...
	class YITH_WCAN_Session_Data_Store {
		...
		public function create( &$session ) {
			...
			if ( $res ) {
				...
				/**
				 * DO_ACTION: yith_wcan_new_session
				 *
				 * Triggered after a new session object is created in db.
				 *
				 * @param int   $id  Session id.
				 * @param array $data Data used to create record in database.
				 */
		...
		public function update( &$session ) {
			...
			/**
			 * DO_ACTION: yith_wcan_update_session
			 *
			 * Triggered immediately after a session object is updated in db.
			 *
			 * @param int               $id   Session id.
			 * @param YITH_WCAN_Session $data Data used to update record in database.
			 */
		...
		public function delete( &$session ) {
			...
			/**
			 * DO_ACTION: yith_wcan_before_delete_session
			 *
			 * Triggered before deleting a session from databases.
			 *
			 * @param int               $id      Id of the session being deleted.
			 * @param YITH_WCAN_Session $session Session object being deleted.
			 */
			...
			if ( $res ) {
				/**
				 * DO_ACTION: yith_wcan_delete_session
				 *
				...
				/**
				 * DO_ACTION: yith_wcan_deleted_session
				 *
		...
		public function get_session_duration() {
			/**
			 * APPLY_FILTERS: yith_wcan_session_duration
			 *
			 * Allow to filter default session TTL
			 *
			 * @param int $ttl Default ttl: 7 days.
			 *
			 * @return int
			 */
		...
		protected function clear_cache( &$session ) {
			if ( $session instanceof YITH_WCAN_Session ) {
				...
				$session = YITH_WCAN_Sessions_Factory::get_session_by_id( $session );
				$hash    = $session ? $session->get_hash() : false;
			} else {
				$session = YITH_WCAN_Sessions_Factory::get_session_by_token( $session );
				$hash    = $session ? $session->get_hash() : false;
			}
