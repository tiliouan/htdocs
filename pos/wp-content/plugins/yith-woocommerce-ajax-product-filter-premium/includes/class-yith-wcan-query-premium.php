...
if ( ! class_exists( 'YITH_WCAN_Query_Premium' ) ) {
	...
	class YITH_WCAN_Query_Premium extends YITH_WCAN_Query_Extended {
		...
		/**
		 * List of supported taxonomies for the plugin
		 *
		 * @var array
		 */
		...
		public function get_supported_taxonomies() {
			/**
			 * APPLY_FILTERS: yith_wcan_pre_get_supported_taxonomies
			 *
			 * Allow third party code to hijack plugins behaviour and provide a list of supported taxonomies,
			 * before plugin retrieve its own list.
			 *
			...
			if ( empty( $this->_supported_taxonomies ) ) {
				...
				/**
				 * APPLY_FILTERS: yith_wcan_excluded_taxonomies
				 *
				 * Filters list of taxonomies excluded from filtering.
				 *
				 * @param array $excluded List of excluded taxonomies.
				 *
				 * @return array
				 */
		...
		public function get_active_filter( $filter ) {
			...
			switch ( $filter ) {
				case 'price_range':
					...
					break;
				case 'orderby':
					$supported_orders = YITH_WCAN_Filters_Factory::get_supported_orders();
		...
		public function get_meta_query( $meta_query = array() ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_product_query_meta_query
			 *
			 * Filter meta query parameters in the filtered query.
			 *
			 * @param array $meta_query Meta query parameter.
			 *
			 * @return array
			 */
		...
		public function get_post_in( $post_in = array() ) {
			...
			if ( $in_stock_only || apply_filters( 'yith_wcan_hide_out_of_stock_products',  'yes' === yith_wcan_get_option( 'yith_wcan_hide_out_of_stock_products', 'no' ), $this, $post_in ) ) {
		...
		public function count_query_relevant_on_sale_products() {
			$count = YITH_WCAN_Cache_Helper::get_for_current_query( 'products_on_sale_count' );

			if ( false === $count ) {
				/**
				 * APPLY_FILTERS: yith_wcan_query_relevant_on_sale_products
				 *
				 * Filter list of on-sale products that match current filters
				 *
				 * @param array $match Array of matched product ids
				 * @return array
				 */
				$products = apply_filters( 'yith_wcan_query_relevant_on_sale_products', array_intersect( $this->get_filtered_products(), $this->get_product_ids_on_sale() ) );
				$count    = count( $products );

				YITH_WCAN_Cache_Helper::set_for_current_query( 'products_on_sale_count', $count );
			}
		...
		public function count_query_relevant_in_stock_products() {
			$count = YITH_WCAN_Cache_Helper::get_for_current_query( 'products_in_stock_count' );

			if ( false === $count ) {
				/**
				 * APPLY_FILTERS: yith_wcan_query_relevant_in_stock_products
				 *
				 * Filter list of in-stock products that match current filters
				 *
				 * @param array $match Array of matched product ids
				 * @return array
				 */
				$products = apply_filters( 'yith_wcan_query_relevant_in_stock_products', array_intersect( $this->get_filtered_products(), $this->get_product_ids_in_stock() ) );
				$count    = count( $products );

				YITH_WCAN_Cache_Helper::set_for_current_query( 'products_in_stock_count', $count );
			}
		...
		public function count_query_relevant_featured_products() {
			$count = YITH_WCAN_Cache_Helper::get_for_current_query( 'products_featured_count' );

			if ( false === $count ) {
				/**
				 * APPLY_FILTERS: yith_wcan_query_relevant_featured_products
				 *
				 * Filter list of featured products that match current filters
				 *
				 * @param array $match Array of matched product ids
				 * @return array
				 */
				$products = apply_filters( 'yith_wcan_query_relevant_featured_products', array_intersect( $this->get_filtered_products(), $this->get_product_ids_featured() ) );
				$count    = count( $products );

				YITH_WCAN_Cache_Helper::set_for_current_query( 'products_featured_count', $count );
			}
		...
		public function count_query_relevant_rated_products( $rate ) {
			$count = YITH_WCAN_Cache_Helper::get_for_current_query( 'products_rated_count', $rate );
			...
			if ( false === $count ) {
				...
				/**
				 * APPLY_FILTERS: yith_wcan_query_relevant_rated_products
				 *
				 * Filter list of products with a specific star-rate that match current filters
				 *
				 * @param array $match Array of matched product ids
				 * @param int   $rate  Rate that products must match.
				 *
				 * @return array
				 */
				$products = apply_filters( 'yith_wcan_query_relevant_rated_products', array_intersect( $this->get_filtered_products_by_query_vars( $query_vars ), get_objects_in_term( $term->term_id, 'product_visibility' ) ), $rate );
				$count    = count( $products );

				YITH_WCAN_Cache_Helper::set_for_current_query( 'products_rated_count', $count, $rate );
			}
		...
		public function count_query_relevant_price_range_products( $range ) {
			$cache_index = md5( http_build_query( $range ) );
			$count       = YITH_WCAN_Cache_Helper::get_for_current_query( 'products_in_price_range_count', $cache_index );
			...
			if ( false === $count ) {
				...
				/**
				 * APPLY_FILTERS: yith_wcan_query_relevant_price_range_products
				 *
				 * Filter list of products within a specific price-range that match current filters
				 *
				 * @param array $match Array of matched product ids
				 * @param int   $range Target price range.
				 *
				 * @return array
				 */
				$products = apply_filters( 'yith_wcan_query_relevant_price_range_products', $wpdb->get_col( $query ), $range ); // phpcs:ignore WordPress.DB.PreparedSQL, WordPress.DB.DirectDatabaseQuery
				$count    = count( $products );

				YITH_WCAN_Cache_Helper::set_for_current_query( 'products_in_price_range_count', $count, $cache_index );
			}
		...
		public function get_query_relevant_min_price() {
			return $this->get_query_relevant_price_limit( 'min' );
		}
		...
		public function get_query_relevant_max_price() {
			return $this->get_query_relevant_price_limit( 'max' );
		}

		/**
		 * Returns minimum/maximum price among currently queried products.
		 * If it cannot determine current query, it will return absolute minimum/maximum price.
		 *
		 * @return int Minimum/Maximum price found
		 */
		protected function get_query_relevant_price_limit( $type = 'min' ) {
			$type        = 'min' === $type ? $type : 'max';
			$limit_price = YITH_WCAN_Cache_Helper::get_for_current_query( "{$type}_price" );

			if ( false === $limit_price ) {
				global $wpdb;
				...
				$lookup_query = "SELECT {$type}({$type}_price) FROM {$wpdb->prefix}wc_product_meta_lookup";

				if ( ! empty( $post_in ) ) {
					$lookup_query .= ' WHERE product_id IN ( ' . implode( ',', array_map( 'esc_sql', $post_in ) ) . ' )';
				}

				$limit_price = $wpdb->get_var( $lookup_query ); // phpcs:ignore WordPress.DB.PreparedSQL, WordPress.DB.DirectDatabaseQuery
				...
				if ( wc_tax_enabled() && ! wc_prices_include_tax() && 'incl' === $tax_display_mode ) {
					...
					if ( $tax_rates ) {
						$limit_price += WC_Tax::get_tax_total( WC_Tax::calc_exclusive_tax( $limit_price, $tax_rates ) );
					}
				}

				$limit_price = 'min' === $type ? floor( $limit_price ) : ceil( $limit_price );
				YITH_WCAN_Cache_Helper::set_for_current_query( "{$type}_price", $limit_price );
			}

			/**
			 * APPLY_FILTERS: yith_wcan_query_relevant_{$type}_price
			 *
			 * Filter maximum price of the products matching current filters selection.
			 *
			 * @param string $type        Type of limit: min/max
			 * @param int  $limit_price Maximum/Minimum price for the matching products.
			 * @return int
			 */
			return apply_filters( "yith_wcan_query_relevant_{$type}_price", $limit_price );
		}
	}
}
