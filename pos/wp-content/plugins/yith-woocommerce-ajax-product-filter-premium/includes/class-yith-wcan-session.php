	...
	class YITH_WCAN_Session extends WC_Data {
		...
		public function __construct( $session = 0 ) {
			...
			$this->data = array(
				...
				'token'      => '',
				/**
				 * APPLY_FILTERS: yith_wcan_default_session_url
				 *
				...
				/**
				 * APPLY_FILTERS: yith_wcan_default_session_query_vars
				 *
				 * Filters default query vars used for sessions.
				 *
				 * @param array $query_vars Default: empty array.
				 *
				 * @return array
				 */
		...
		public function get_share_url() {
			...
			if ( $wp_rewrite->using_mod_rewrite_permalinks() ) {
				$share_url = trailingslashit( $origin_url ) . YITH_WCAN_Sessions_Factory::get_session_query_param() . "/{$token}/";
			} else {
				$share_url = add_query_arg( YITH_WCAN_Sessions_Factory::get_session_query_param(), $token, $origin_url );
			}

			/**
			 * APPLY_FILTERS: yith_wcan_session_share_url
			 *
		...
		public function generate_hash() {
			...
			$hash = YITH_WCAN_Sessions_Factory::calculate_hash( $origin_url, $query_vars );
		...
		public function is_expiring() {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_expiring_threshold
			 *
			 * Number of seconds of life for any giver session.
			 *
			 * @param int               $ttl        Session ttl.
			 * @param int               $session_id Id of current session.
			 * @param YITH_WCAN_Session $session    Current session.
			 *
			 * @return int
			 */
		...
		public function maybe_extend_duration() {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_expiration_increment
			 *
			 * Filters number of seconds that should be added to a session expiration time.
			 *
			 * @param int               $increment  Number of seconds.
		...
		public function save() {
			if ( $this->data_store ) {
				 ...
				 *
				 * @param YITH_WCAN_Session $this Current object.
