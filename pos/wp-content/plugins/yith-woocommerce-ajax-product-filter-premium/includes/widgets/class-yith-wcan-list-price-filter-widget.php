...
if ( ! class_exists( 'YITH_WCAN_List_Price_Filter_Widget' ) ) {
	...
	class YITH_WCAN_List_Price_Filter_Widget extends WP_Widget {
		...
		public function form( $instance ) {
            ...
            // Retrieve the min price
			$value = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
				$wpdb->prepare(
					"SELECT min(meta_value + 0) FROM {$wpdb->posts} as p LEFT JOIN {$wpdb->postmeta} as pm ON p.ID = pm.post_id WHERE meta_key IN (" . str_repeat( '%s ',
						count( $price_meta ) ) . ')', // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					$price_meta
				)
			);

			$min = isset( $value ) ? floor( $value ) : 0;
